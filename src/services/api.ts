...
export const createUser = async (data: Omit<User, 'id' | 'userType' | 'walletBalance' | 'totalRides' | 'totalSpent'>): Promise<User> => {
  await delay();
  const user: User = { ...data, id: Date.now(), userType: 'user', walletBalance: 0, totalRides: 0, totalSpent: 0, createdAt: new Date().toISOString() };
  db.users.push(user);
  return user;
};
export const updateUser = async (id: number, patch: Partial<User>): Promise<User> => {
  await delay();
...
export const createRide = async (data: Omit<Ride, 'id'>): Promise<Ride> => {
  await delay();
